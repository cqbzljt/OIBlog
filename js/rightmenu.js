...
kk.changeMessage = function(message, timeout=1000, bgc='var(--font-color)', color='var(--kk-card-bg)') {
  let item = $('#message-wrap .message-item');
  if(!kk.messageTimeout){
    item.fadeIn(400);
  }
  item.html(message);
  if(bgc){item.css('backgroundColor',bgc);}
  if(color){item.css('color',color);}
  clearTimeout(kk.messageTimeout);
  kk.messageTimeout = setTimeout(function(){item.fadeOut(300);kk.messageTimeout = null},timeout);
}
...
kk.switchTheme=function(load=false){
  ...
  let themesrc = ''
  if(themes[vTheme]){
      themesrc += 'https://adn.arcitcgn.cn/npm/saiodgm-oiblog@1.3.1/css/themes/'+themes[vTheme];
  }
...
{
  $('.copy-button').on('click',function(){kk.changeMessage("复制成功", 1000, 'var(--card-bg)','var(--font-color)')})
}